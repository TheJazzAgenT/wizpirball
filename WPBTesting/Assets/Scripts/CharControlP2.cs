...
public class CharControlP2 : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        LeftBarr = GameObject.Find("ShieldActivatePoint1_P2");
        RightBarr = GameObject.Find("ShieldActivatePoint2_P2");
        FrontBarr = GameObject.Find("ShieldActivatePoint3_P2");
        BackBarr = GameObject.Find("ShieldActivatePoint4_P2");
        manaCost = 0;
        manaRegen = Regen();
    ...
    private void Update()
    {
        if ((Input.GetKeyDown(KeyCode.Mouse0) || Input.GetAxis("RightTrigger_2") > 0) && Time.time >= timestamp)
        {
            Debug.Log("TRIGGERED");
        ...
        if (Input.GetKeyDown(KeyCode.Alpha1) || Input.GetButtonDown("A_2"))
        {
            //Debug.Log("AAAAAAAAAAAAAAAAAAAAAAAAAA");
            bulletPrefab = Bullets[0];//normal
            manaCost = 0;
        }
        if (Input.GetKeyDown(KeyCode.Alpha2) || Input.GetButtonDown("B_2"))
        {
            bulletPrefab = Bullets[1];//fire
            manaCost = 15;
        }
        if (Input.GetKeyDown(KeyCode.Alpha3) || Input.GetButtonDown("X_2"))
        {
            bulletPrefab = Bullets[2];//ice
            manaCost = 25;
        }
        if (Input.GetKeyDown(KeyCode.Alpha4) || Input.GetButtonDown("Y_2"))
        {
    ...
    {
        if (col.transform.tag == "ShieldActivator" && (Input.GetKeyDown(KeyCode.Space) || Input.GetButtonDown("RightBumper_2")))
        {
    ...
    public void Fire()
    {
        if (mana > manaCost)
        {
            ...
            bullet.GetComponent<PlayerSelector>().SetPlayer("P2", "P1");
            mana -= manaCost;
            bar.fillAmount = mana;
