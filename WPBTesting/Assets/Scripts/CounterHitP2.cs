...
public class CounterHitP2 : MonoBehaviour
{
    ...
    void OnTriggerStay(Collider other)
    {
        if (other.tag == "Bullet")
        {
            ...
            if ((Input.GetKeyDown(KeyCode.Mouse1) || (Input.GetAxis("LeftTrigger_2") > 0)) && timer > counterDelay)
            {
                //throwBat = (GameObject)Instantiate(bat, batSpawn.position, batSpawn.rotation);
                myCharacter.GetComponent<CharControlP2>().Fire();
